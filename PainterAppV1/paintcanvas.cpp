...
void PaintCanvas::drawRect(const QPoint &_startPoint, const QPoint &_endPoint, bool _elipse)
{
    drawing = false;
    lastPoint = _startPoint;
    drawRectTo(_endPoint, _elipse);
    lastPoint = {0, 0};
}

void PaintCanvas::drawLine(const QPoint &_startPoint, const QPoint &_endPoint)
{
    lastPoint = _startPoint;
    drawing = true;
    drawLineTo(_endPoint);
    drawing = false;
    lastPoint = {0,0};
}
...
void PaintCanvas::drawLineTo(const QPoint &endPoint)
{
    ...
    qDebug() << "drawline(" << endPoint.x() << "," << endPoint.y() << ")\n";
...
QPoint PaintCanvas::drawLineTo(const QPoint &_lastPoint ,const QPoint &endPoint)
{
    QPainter painter(&image);
    painter.setPen(QPen(penColor, penWidth, Qt::SolidLine, Qt::RoundCap,
                        Qt::RoundJoin));
...
void PaintCanvas::drawRectTo(const QPoint &endPoint, bool elipse)
{
    ...
    if(!elipse)
        ...
        QSize mysize(endPoint.x(), endPoint.y());

        painter.drawEllipse(QRect(lastPoint,mysize));
    }
